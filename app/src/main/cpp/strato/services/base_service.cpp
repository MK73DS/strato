...
namespace strato::service {
    const std::string &BaseService::GetName() {
        if (name.empty()) {
            ...
            name = (status == 0) ? std::string(demangled.get() + std::char_traits<char>::length("strato::service::")) : mangledName;
        }
        return name;
    }
    ...
    Result service::BaseService::HandleRequest(type::KSession &session, ipc::IpcRequest &request, ipc::IpcResponse &response) {
        ...
        try {
            ...
            // We need to forward any strato::exception objects without modification even though they inherit from std::exception
