...
#define LOG_WRITE(level, ...)                                                               \
        do {                                                                                \
            if (!strato::AsyncLogger::CheckLogLevel(level))                                \
                break;                                                                      \
            std::string _str = fmt::format(__VA_ARGS__);                                    \
            strato::AsyncLogger::LogAsync(level, std::move(_str), __builtin_FUNCTION());   \
        } while (0)
...
#define LOGNF_WRITE(level, ...)                                                             \
        do {                                                                                \
            if (!strato::AsyncLogger::CheckLogLevel(level))                                \
                break;                                                                      \
            std::string _str = fmt::format(__VA_ARGS__);                                    \
            strato::AsyncLogger::LogAsync(level, std::move(_str), nullptr);                \
        } while (0)

/**
 * @brief Logs an Error message, formatted using fmt::format
 */
#define LOGE(...) LOG_WRITE(strato::AsyncLogger::LogLevel::Error, __VA_ARGS__)
/**
 * @brief Logs an Error message without the calling function name, formatted using fmt::format
 */
#define LOGENF(...) LOGNF_WRITE(strato::AsyncLogger::LogLevel::Error, __VA_ARGS__)

/**
 * @brief Logs a Warning message, formatted using fmt::format
 */
#define LOGW(...) LOG_WRITE(strato::AsyncLogger::LogLevel::Warning, __VA_ARGS__)
/**
 * @brief Logs a Warning message without the calling function name, formatted using fmt::format
 */
#define LOGWNF(...) LOGNF_WRITE(strato::AsyncLogger::LogLevel::Warning, __VA_ARGS__)

/**
 * @brief Logs an Info message, formatted using fmt::format
 */
#define LOGI(...) LOG_WRITE(strato::AsyncLogger::LogLevel::Info, __VA_ARGS__)
/**
 * @brief Logs an Info message without the calling function name, formatted using fmt::format
 */
#define LOGINF(...) LOGNF_WRITE(strato::AsyncLogger::LogLevel::Info, __VA_ARGS__)

/**
 * @brief Logs a Debug message, formatted using fmt::format
 */
#define LOGD(...) LOG_WRITE(strato::AsyncLogger::LogLevel::Debug, __VA_ARGS__)
/**
 * @brief Logs a Debug message without the calling function name, formatted using fmt::format
 */
#define LOGDNF(...) LOGNF_WRITE(strato::AsyncLogger::LogLevel::Debug, __VA_ARGS__)

/**
 * @brief Logs a Verbose message, formatted using fmt::format
 */
#define LOGV(...) LOG_WRITE(strato::AsyncLogger::LogLevel::Verbose, __VA_ARGS__)
/**
 * @brief Logs a Verbose message without the calling function name, formatted using fmt::format
 */
#define LOGVNF(...) LOGNF_WRITE(strato::AsyncLogger::LogLevel::Verbose, __VA_ARGS__)
